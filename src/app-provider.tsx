...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const rooms: Room[] = Array.from({ length: TOTAL_ROOMS }, (_, i) => {
    ...
    const occupyingAppointment = appointments?.find(
     ...
     // A room is considered assigned if a patient is on their way
    const assignedAppointment = appointments?.find(
      (apt) => apt.status === 'InRoom' && apt.assignedRoomNumber === roomNumber
    );

    const currentAppointment = occupyingAppointment || assignedAppointment;
    const patient = currentAppointment && patients ? getPatientById(currentAppointment.patientId) : undefined;
    
    let status: Room['currentStatus'] = 'Available';
    if(occupyingAppointment) status = 'Occupied';
    else if (assignedAppointment) status = 'Assigned';
